...
### マスタのローカルキャッシュ — 実装（M6・完了）

| 対象 | 方式 |
|---|---|
| 設備台帳 / 点検表 / 職場 | 一覧系。起動時とオンライン復帰時に**全置換** |
| 正常見本メディア / 点検部位マップ（図面ピン） | **点検開始時に該当分だけ先読み** |

公開 API は `core.master`（`createMasterCache`）と
...
type MediaAvailability =
  | { state: "cached"; url; bytes }   // 端末にある。圏外でも出る
  | { state: "online"; url }          // 端末に無いが、いま取りに行ける
  | { state: "unavailable"; reason }  // → 「オンライン時に表示されます」
  | { state: "loading" };
```

`unavailable` を「読み込み失敗」「画像がありません」と出してはいけない。
現場ではデータの不備に見え、別の問い合わせになる。

実装で確定した点:

- **容量の枠はキューと別に持つ**（既定60MB）。
  マスタは取り直せるので容量が苦しければ古い順に捨ててよい。
  未送信ジョブは端末にしか無いので絶対に捨てられない。
  この判断が逆なので、同じ枠で数えると「あと何枚撮れるか」が読めなくなる
- **取得に失敗しても既存のキャッシュを消さない。** 圏外で点検を開始できることが優先
- **先読みは括り（点検表ID等）単位**。次の点検に移ったらまとめて捨てられる
- **1枚取れなくても点検は開始できる**。失敗件数を返すだけで例外にしない
- **`navigator.onLine` を信じない。** 真でも実際には通らないことがあるので、
  `<img onError>` から `reportUnavailable()` を呼んで
  「オンライン時に表示されます」へ落とす経路を用意した。
  実際、`onLine` の反転と `offline` イベントの順序は環境によって競合する
- 変更は購読で伝える。起動時の取り直しは非同期なので、
  画面がその途中で開かれても取り残されないようにする

### マスタのローカルキャッシュ（M6・アプリ側の回答を反映）

圏外で**点検を新規に開始する**ために要る。M5 の時点では対象が未確定だったため
推測で作らず、アプリ側の回答を待った。以下が確定した要件（pf-setsubi の場合）。

| 対象 | 内容 |
|---|---|
| 設備台帳 | 管理番号・名称・職場・設置場所 |
| 点検表マスタ | 点検項目・基準値・判定条件 |
| 職場マスタ | — |

**粒度と更新方針**

- 取得範囲は**ユーザーの所属する会社・工場に限定**する（全社分は持たない）
- 更新の契機は**起動時とオンライン復帰時**
- 更新は差分ではなく**全置換**でよい（件数が少ないため、
  差分同期の複雑さに見合わない）
- 規模の目安: 設備3台・点検表4種。将来的にも数百件程度

この規模なら IndexedDB に素直に置ける。**画像と違って容量の心配が要らない**ので、
滞留上限（`retention`）の対象にはしない — 未送信ジョブの容量計算に
マスタを混ぜると、写真を預かれる量が読めなくなる。

取得範囲を会社・工場で絞るのは容量のためだけではない。
全社分を端末に置くと、テナント分離が**端末の中**で崩れる。
サーバ側の `authorize()` で絞ったものだけを返すこと。
