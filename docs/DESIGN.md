...
> `SUPABASE_SERVICE_ROLE_KEY` は **サーバー環境変数としてのみ**扱う。
...
| **M4** | React バインディング（`react/`, Provider, `useSignedUrl`）+ **pf-setsubi パイロット** | playground の UI で未送信件数・手動再送・進捗・画像表示が動く。<br>pf-setsubi で: `@vercel/blob` からの置換と `provider: 'vercel-blob'` の並存、<br>`blocked(auth)` からの再ログイン導線、無操作ログアウトの調停（§5-8）まで含めて実地投入 |
| **M5** | Service Worker（`sw/`, `cli/`, Background Sync, 署名メディアのキャッシュ正規化）| アプリを閉じた状態から Background Sync で送信完了。圏外でアプリシェルが起動 |
...
### 5-8. 追加で判断が要る事項（認証調査で新たに判明）

`docs/auth-findings.md` の調査で、当初の想定に無かった論点が3つ出た。
**いずれも M4（pf-setsubi パイロット）までに決まればよく、M0〜M3 は着手できる。**

1. **無操作ログアウトと未送信ジョブの調停** — 共用端末は15分でログアウトし Cookie が破棄されるため、
   未送信ジョブが送れなくなる。推奨は「未送信がある間は `useIdleLogout` を抑止（要 pf-ui 変更）+
   ログアウト直前に `flush()`」。
2. **既存 `@vercel/blob` データの扱い** — `StoredObjectRef.provider` で並存（推奨・移行バッチ不要）か、
   一括コピーして `blobUrl` を廃止するか。
3. **オフラインでの新規入力開始** — 現設計は「入力開始はオンライン、送信だけオフライン耐性」。
   圏外で点検入力を開始できる必要があるなら、マスタデータのローカルキャッシュ設計が別途要り、
   スコープが一段広がる。
